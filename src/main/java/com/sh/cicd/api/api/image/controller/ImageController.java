...
public class ImageController {
    ...
    public ApiResponse<ImageUrlResponse> getPresignedUrlOfUserUpload(
            @PathVariable Long userId, @RequestParam String filename) {
        return ApiResponse.success(
                SuccessResults.OK, getPresignedUrlUseCase.getPresignedUrlForUser(filename));
    }
    ...
    public ApiResponse<ImageDto> userUploadImageSuccess(
            @PathVariable Long userId, @RequestBody SuccessUserImageUploadRequest request) {
        return ApiResponse.success(
                SuccessResults.CREATED,
                imageUploadSuccessUseCase.userUploadImageSuccess(userId, request.getImageKey()));
    }

    // 유저 이미지 삭제
    @DeleteMapping("/users/{userId}")
    @ResponseStatus(HttpStatus.OK)
    public ApiResponse<ImageDto> deleteImageForUser(@PathVariable Long userId) {
        return ApiResponse.success(
                SuccessResults.DELETED, imageDeleteUseCase.deleteImageForUser(userId));
    }
}
